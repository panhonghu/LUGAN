...
class MirrorPoses(object):
    ...
    def __call__(self, data):  # T*17*3
        # data[:, :, 0] -> T*17
        if np.random.random() <= self.probability:
            center = np.mean(data[:, :, 0], axis=1, keepdims=True)  # T*1
...
class JointNoise(object):
    ...
    def __call__(self, data):
        # T, V, C
        noise = np.hstack((np.random.normal(0, 0.25, (data.shape[1], 2)),
                           np.zeros((data.shape[1], 1)))).astype(np.float32)
...
class InterpolateFrames(object):
    ...
    def __call__(self, data):
        ...
        for i in range(T):
            ...
            new_frame_x = (data[i, :, 0] + (x_difference * np.random.normal(0.5, 1)))
            new_frame_y = (data[i, :, 1] + (y_difference * np.random.normal(0.5, 1)))
            new_frame_conf = (data[i + 1, :, 2] + data[i, :, 2]) / 2
            interpolated_frame = np.array([new_frame_x, new_frame_y, new_frame_conf]).transpose()
